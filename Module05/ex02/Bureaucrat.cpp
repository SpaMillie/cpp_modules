...
/*   Updated: 2024/12/14 17:14:09 by mspasic          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void        Bureaucrat::signForm(AForm& obj, std::string str) const{
    if (obj.getState() == "true" && str.size() == 0)
        std::cout << this->_name << " signed " << obj.getName() << "'s form\n";
    else{
        std::cout << this->_name << " couldn't sign " << obj.getName() << "'s form because " << str;
    }
}

void    Bureaucrat::executeForm(AForm const& form) const{
    if (form.getState() == "true" && _grade < form.getGradeExec()){
